...
export interface ReferenceReturnType {
  ...
  status: string;
  trade_token: string;
  dynamic_link?: string; // opcional, pois REF nem sempre retorna
}

export default class ReferencePayment implements PaymentStrategie {
  private loger = new Logger('ReferencePayment Logger');
  ...
  public async pay(amount: string): Promise<IGetawayReponse> {
    const tradeNo = `ORDER_${Date.now()}`; // único para trade_info e request_no
    ...
    // Gera a assinatura **antes** do encodeURIComponent
    const signature = this.generateSignature(requestBody);

    // Codifica os campos (exceto sign e sign_type)
    ...
    // REF nem sempre retorna dynamic_link, então não lança erro
    return {
      entitie: data.entity_id,
      reference: data.reference_id,
      payUrl: data.dynamic_link ?? '', // opcional
